...
@endif
    <div class="card body">
        <form method="post" action="/update/{{ $tamu->id }}" enctype="multipart/form-data">
            @csrf
                <div class="card mb-3">
                ...
                <input type="text" name="nama" class="form-control" id="exampleInputName" aria-describedby="name"
                    placeholder="Masukan nama anda" value="{{ $tamu->nama }}">
                </div>
                <div class="card mb-3">
                <label class="input-group-text" for="exampleInputPassword1">NIK</label>
                <input type="enter number" name="nik" class="form-control" id="exampleInputNIK" placeholder="NIK" value="{{ $tamu->nik }}">
                </div>
                <div clas="card mb-3">
                ...
                <input type="text" name="alamat" class="form-control" id="exampleInputAdress" aria-describedby="adress"
                    placeholder="Masukan alamat anda" value="{{ $tamu->alamat }}">
                </div>
                <div clas="card mb-3">
                    ...
                    <input type="text" name="instansi" class="form-control" id="exampleInputAdress" aria-describedby="adress"
                        placeholder="Masukan Instansi anda" value="{{ $tamu->instansi }}">
                    </div>
                <div class="card mb-3">
                    <label class="input-group-text" for="inputState" class="form-label">Tanggal Berkunjung</label>
                    <input type="date" name="tanggal" class="form-control" id="simpleDataInput" aria-describedby="date" value="{{ $tamu->tanggal }}">
                </div>
                <div class="card mb-3">
                    ...
                    <option selected>{{ $tamu->bidang_terkait }}</option>
                ...
                <div class="card mb-3">
                    ...
                    <input type="text" name="keperluan" class="form-control" id="exampleInputAdress" aria-describedby="adress"
                        placeholder="Masukan keperluan anda" value="{{ $tamu->keperluan }}">
                    </div>
                <div class="card mb-3">
                    <label class="input-group-text" for="formFile" class="form-label">Foto KTP</label>
                    <input class="form-control" name="foto_ktp" type="file" id="formFile" value="{{ $tamu->foto_ktp }}">
                </div>
                <div class="card mb-3">
